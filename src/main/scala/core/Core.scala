...
import rainy.shaheway.org.core.backend.regfile.{CSRReadPort, CSRfile, RegFile}
import rainy.shaheway.org.mem.{DataReadPort, InstReadPort, MemWritePort}
class Core extends Module {
  ...
  instfetch.io.branchTarget := tmp
  instfetch.io.jumpTarget := tmp
  instfetch.io.fetchMem <> io.instfetch_fetchMem
  instfetch.io.envRead <> ?
  // instfetch_to_decode
  ...
  instfetch_to_decode.io.jumpOrBranchFlag := tmp
  // decode
  ...
  writeback.io.regWrite <> ?
  writeback.io.csrWrite <> ?
